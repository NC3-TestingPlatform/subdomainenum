...
class TestAssess:
    ...
    def test_passive_mode_skips_active_tools(self) -> None:
        ...
        mock_active_enum.assert_not_called()
    ...
    def test_deduplicates_subdomains_across_sources(self) -> None:
        sources = [
            _make_source("sub.example.com", name="findomain"),
    ...
    def test_active_mode_uses_active_enum_ips(self) -> None:
        """Gobuster-discovered FQDNs' IPs must feed ffuf in ACTIVE mode."""
        active_src = _make_source("git.example.com", name="gobuster")
        with (
            patch("subdomainenum.assessor._run_active_enum", return_value=[active_src]),
            patch(
                "subdomainenum.assessor._run_ffuf_fanout",
                return_value=(ToolResult(name="ffuf", mode=EnumMode.ACTIVE), []),
            ) as mock_ffuf,
...
class TestRunActive:
    def test_returns_sources_without_urls(self) -> None:
        """Active pool is gobuster plus the ffuf sentinel marked unavailable
        when no URLs are provided → 2 sources."""
        src = _make_source()
        with (
            patch("subdomainenum.assessor.run_gobuster_dns", return_value=src),
            patch("subdomainenum.assessor.run_ffuf") as mock_ffuf,
        ):
            enum_tools = _run_active_enum("example.com", wordlist="/tmp/w.txt", progress_cb=None)
            ffuf_tool, _ = _run_ffuf_fanout("example.com", wordlist="/tmp/w.txt", urls=[], progress_cb=None)
            sources = enum_tools + [ffuf_tool]
    ...
    def test_runs_ffuf_when_urls_provided(self) -> None:
        ...
        with (
            ...
            _run_active_enum("example.com", wordlist="/tmp/w.txt", progress_cb=None)
            _run_ffuf_fanout("example.com", wordlist="/tmp/w.txt", urls=["http://example.com"], progress_cb=None)
    ...
    def test_progress_cb_called(self) -> None:
        ...
        with patch("subdomainenum.assessor.run_gobuster_dns", return_value=src):
            _run_active_enum("example.com", wordlist="/tmp/w.txt", progress_cb=calls.append)
            _run_ffuf_fanout("example.com", wordlist="/tmp/w.txt", urls=[], progress_cb=calls.append)
        assert len(calls) > 0
    ...
    def test_debug_cb_lambda_invoked(self) -> None:
        ...
        with patch("subdomainenum.assessor.run_gobuster_dns", side_effect=fake_gobuster):
            _run_active_enum(
                "example.com",
                wordlist="/tmp/w.txt",
                progress_cb=None,
            ...
            _run_ffuf_fanout("example.com", wordlist="/tmp/w.txt", urls=[], progress_cb=None)
    ...
    def test_finish_cb_called_for_ffuf(self) -> None:
        ...
        with (
            ...
            _run_active_enum("example.com", wordlist="/tmp/w.txt", progress_cb=None, finish_cb=finish_cb)
            _run_ffuf_fanout(
                "example.com", wordlist="/tmp/w.txt", urls=["http://example.com"], progress_cb=None, finish_cb=finish_cb
            )
        names = [n for n, _, _ in finish_calls]
        assert "ffuf" in names

    def test_active_sources_have_mode_active(self) -> None:
        """All ToolResults from the active phase have mode=EnumMode.ACTIVE stamped."""
        src = _make_source()
        with patch("subdomainenum.assessor.run_gobuster_dns", return_value=src):
            enum_tools = _run_active_enum("example.com", wordlist="/tmp/w.txt", progress_cb=None)
            ffuf_tool, _ = _run_ffuf_fanout("example.com", wordlist="/tmp/w.txt", urls=[], progress_cb=None)
...
class TestPhaseAwareKeys:
    ...
    def test_active_active_mode_plain_keys(self) -> None:
        """In ACTIVE-only mode no suffix is added; pool is gobuster only."""
        names = self._active_finish_names(EnumMode.ACTIVE)
        assert "gobuster" in names

    def test_active_no_overall_mode_plain_keys(self) -> None:
        """Without overall_mode (None) no suffix is added; pool is gobuster only."""
    ...
    def test_assess_passes_overall_mode_all_to_passive(self) -> None:
        with (
            ...
            patch(
                "subdomainenum.assessor._run_ffuf_fanout",
                return_value=(ToolResult(name="ffuf", mode=EnumMode.ALL), []),
            ),
            patch("subdomainenum.assessor._resolve_all", return_value=[]),
            patch("subdomainenum.assessor.resolve_ips", return_value=[]),
        ):
    ...
    def test_assess_passes_overall_mode_all_to_active(self) -> None:
        """In ALL mode, overall_mode is forwarded to _run_active_enum."""
        with (
            patch("subdomainenum.assessor._run_passive_enum", return_value=[]),
            patch("subdomainenum.assessor._run_active_enum", return_value=[]) as mock_active_enum,
            patch(
...
class TestActiveParallelism:
    """The non-ffuf active pool is gobuster-only."""

    def test_active_tool_runs(self) -> None:
        """The active pool is gobuster-only. Verify gobuster runs and returns a ToolResult."""
        with patch(
            "subdomainenum.assessor.run_gobuster_dns",
            return_value=_make_source(name="gobuster"),
        ):
            tools = _run_active_enum("example.com", wordlist="/tmp/w.txt", progress_cb=None)
        assert len(tools) == 1
        assert tools[0].name == "gobuster"
    ...
    def test_active_dnsrecon_always_absent_from_active_pool(self) -> None:
        ...
        with patch("subdomainenum.assessor.run_gobuster_dns", return_value=_make_source(name="gobuster")):
            tools = _run_active_enum(
                "example.com", wordlist="/tmp/w.txt",
                progress_cb=None, overall_mode=EnumMode.ALL,
            )
        names = {t.name for t in tools}
        assert names == {"gobuster"}
    ...
    def test_active_enum_captures_unexpected_exception(self) -> None:
        ...
        with patch("subdomainenum.assessor.run_gobuster_dns", side_effect=RuntimeError("boom")):
            tools = _run_active_enum("example.com", wordlist="/tmp/w.txt", progress_cb=None)
...
class TestAllModePhaseFusion:
    ...
    def test_all_mode_fuses_phases(self) -> None:
        """Passive helpers (4) and active-enum helper (1) all hit a Barrier(5) → fused.
        run_dnsrecon runs only passively (1 hit); gobuster only actively (1 hit)."""
        import threading
